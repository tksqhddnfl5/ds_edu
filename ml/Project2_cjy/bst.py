...
def do_cv():
    ...
    # X_features5, _, Y5, _ = train_test_split(X_features, Y, test_size=0.5, random_state=0)
    ...
    def objective(trial):
        ...
        # 교차 검증 점수로 평가
        scores = cross_val_score(model, X_features, Y, groups=groups, cv=LeaveOneGroupOut(), scoring="f1_macro")
        return scores.mean()
